...
Dim cname,category,seqNo,id, title,operate
	...
	Sub SaveRecord()
		...
		'判断是否已经存在网站名称
		Sql = "select id from T_Facility Where cname='"&cname&"'"
		If id <> "" Then
			Sql = Sql & " and id<>"&id
		End If
		...
		Sql = "Select * from T_Facility Where 1=1"
		If id <> "" and id <> 0 Then
			Sql = Sql & " and ID="&id
		End If
		...
		'gMsgFlag = "S"
		Dream3CLS.showMsg "保存成功","S","facility.asp"
	...
	Sub ShowEdit()	
		...
		sql = "select id,cname,category,seqno from T_Facility Where id="&id
		Set Rs = Dream3CLS.Exec(Sql)
		If Rs.EOF Then
...
<div id="box">
                ...
                <div class="sect">
                    <form method="post" action="facilityEdit.asp?act=save">
