   ...
   Dream3File.CreateAbsoluteFolder(Server.mappath("../../"&filepath))
	...
	for each formName in upload.File
		 ...
		 if file.filesize>(5000*1024) then
				response.write "<span style=""font-family: 宋体; font-size: 9pt"">最大只能上传 5000K 的图片文件！　[ <a href=# onclick=history.go(-1)>重新上传</a> ]</span>"
				response.end
			end if
	...
	filename="../../"&filename&"."&fileExt
		 ...
		 if file.FileSize>0 then         ''如果 FileSize > 0 说明有文件数据
			  ...
			  '检验图片木马
			  attachFlag = Dream3CLS.CheckFileContent(FileName)
			  If attachFlag = True Then
			  	'直接删除图片
				Dream3File.DeleteFile(Server.mappath(FileName)	)
			  End If
