...
Dim username,email,password,confirm,city_id,mobile,subscribe,userid,manager,validcode,affirm,reg_code,hotelname,headname
	...
	Sub SaveRecord()
		...
		telcode = Request.Cookies("telcode")
		if Request.Cookies("telcode")="" Then
			gMsgArr = gMsgArr&"|您没有填写手机验证码"
		else
			if reg_code <> telcode then
				gMsgArr= gMsgArr&"|您填写的手机验证码"&reg_code&"不正确，请重新输入"
			end if		
			Response.Cookies("telcode").Expires= (now()-1)
		end if
    ...
    <div class="reg_cente">
    	<div class="reg_left">
        	<dl>
                ...
                <dd>手机号码：
				<input type="text" value="<%=mobile%>" class="reg_txt2" id="mobile" name="mobile">
				<input name="regcodesub" type="image" src="../../common/themes/Default/img/regcode.jpg" onclick="runcodes(4);return false;" />
				<span class="validatorMsg">请输入你的手机号码</span>
				</dd>
                
				<dd>验证码:
				<br /><input type="text" value="<%=reg_code%>" class="reg_txt3"  name="reg_code" id="reg_code" >
				<span class="validatorMsg"><br />请输入手机验证码</span>
				</dd>
