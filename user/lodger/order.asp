...
Set productMap = new AspMap
	...
	Sub LodgerCancel()
		...
		sql = "select * from T_Order where id = " & orderid & " and user_id=" & Session("_UserID")
		...
		Sql = "Update  T_Order set state = 'lodgercancel' Where id = " & orderid
                        ...
                        <div id="tagContent" class="blue_link">
                            ...
                            <div class="tagContent selectTag" id=tagContent0>
                                <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="house-adr">
									...
									<%
'id,order_no,product_id,city_code,checkindays,state,roomnum,checkintype,royalties,pay_time,create_time			
										For i = 0 to UBound(arrU, 2) 
											s_id = arrU(0,i)
											s_order_no = arrU(1,i)
											...
											s_housetitle = productMap.getV(CStr(s_product_id))(0)
		...
		Set rs = nothing
                                    ...
                                    <tr class="tr">
                                        ...
                                        <td><%=s_state_str%></td>
                                        <td>
										<a href="<%=VirtualPath%>/user/order/view.asp?u=lodger&c=<%=classifier%>&id=<%=s_id%>">详情</a>
										<%
										If s_state = "unconfirm" or s_state = "unpay" Then
										%>
										<a href="?act=lodgercancel&orderid=<%=s_id%>" onclick="return window.confirm('您确定要取消该订单?')">取消</a>
										<%End If%>
										<%
										If s_state = "unpay"  Then
										%>
										<a href="<%=VirtualPath%>/check.asp?id=<%=s_id%>">支付</a>
										<%End If%>
										</td>
                                    </tr>
                                    <%
