...
Sub SaveRecord()
	...
	Sql = "Select * from T_SpecialPrice where product_id="&pid
	
	pRs.open Sql,conn,1,2
	
	counter = 0
	Do While counter < 62 
		Dim DateCount, JudgeSql,delSql
		DateCount = DateAdd("d", counter, date())
		
		'此处用来区分修改房间和新添加房间的specialprice部分，对于specialprice中已有的数据先删除后插入
		JudgeSql = "Select * from T_SpecialPrice where product_id="&pid&" and date >= '"&DateCount&"' and date <'"&DateAdd("d",1,DateCount)&"'"
		Set jRs = Dream3CLS.Exec(JudgeSql)
		  if not jRs.EOF then 
		  delSql = "delete from T_SpecialPrice where product_id="&pid&" and date >= '"&DateCount&"' "
		  Dream3CLS.Exec(delSql)
		  end if 
		jRs.close
		Set jRs = nothing
		...
		If Weekday(DateCount) = 6 or Weekday(DateCount) = 7 then
		pRs("price") = weekrentprice
		Else
		pRs("price") = dayrentprice
		End If
	...
	directPage = "preview.asp?pid="&pid
        ...
        <div class="side-center">
            ...
            <div class="detail-right">
                ...
                <dl>
                    ...
                    <input type="text"  name="dayrentprice" class="radius input" value="<%=dayrentprice%>">元/每晚<span style="color:red">*</span>
                    <span id="tip_daypriceunit"></span>
                    </dd>
                    <dd>
                    <label>周末(周五周六)价：</label>
                    <input type="text" name="weekrentprice" class="radius input" value="<%=weekrentprice%>">元/每晚<span style="color:red">*</span>
